@extends('layouts.app')

@section('title', 'Contato - WebContabil')
@section('page', 'contato')

@section('content')
    <main class="max-w-6xl mx-auto px-4 py-20">
        <h1 class="text-4xl font-bold theme-text-high mb-6">Contato</h1>
        <a href="/" class="inline-flex items-center gap-2 rounded-full px-5 py-3 bg-brand text-white hover:bg-brand-light transition mb-10">
            ← Voltar ao Início
        </a>
        <p class="text-lg theme-text-low leading-relaxed mb-6">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur ac augue vel
            erat pharetra venenatis.
        </p>
        <p class="text-base theme-text-low leading-relaxed">
            Nunc sit amet elit vitae mi finibus vehicula. Sed nec rutrum tortor, sit amet
            accumsan leo. Praesent tristique orci id metus dictum, vitae fermentum purus cursus.
        </p>
    </main>
@endsection
